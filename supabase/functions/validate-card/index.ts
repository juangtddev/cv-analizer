...
serve(async (req) => {
  ...
  try {
    ...
    try {
      ...
      if (paymentIntent.status === 'succeeded') {
        ...
        if (charges.data.length > 0) {
          ...
          return new Response(JSON.stringify({
            success: true,
            message: "Cartão validado com sucesso! O valor de R$ 1,00 foi estornado automaticamente.",
            validation: {
              card_valid: true,
              has_limit: true,
        ...
        logStep("Falha na validação do cartão", { status: paymentIntent.status });
        
        return new Response(JSON.stringify({
          success: false,
          message: "Falha na validação do cartão. Verifique os dados e tente novamente.",
          validation: {
            card_valid: false,
            has_limit: false,
            status: paymentIntent.status
      ...
      } else if (paymentError.code === 'invalid_expiry_month' || paymentError.code === 'invalid_expiry_year') {
